...
# Build backend and frontend separately to reduce peak memory usage.
RUN NODE_OPTIONS="--max-old-space-size=4096" TURBO_CONCURRENCY=1 bunx turbo run build:web --filter=@blinko/backend
# Frontend build is memory-intensive and can OOM in Docker. If SKIP_FRONTEND_BUILD=true,
# expect prebuilt assets to exist at dist/public in the build context.
RUN if [ "$SKIP_FRONTEND_BUILD" = "true" ]; then \
      echo "Skipping frontend build (SKIP_FRONTEND_BUILD=true)" && \
      if [ ! -d "dist/public" ]; then \
        echo "ERROR: dist/public not found. Build frontend on host first (bun run build:no-pwa)" && exit 1; \
      fi; \
    else \
      DISABLE_PWA=true LOW_MEM_BUILD=true NODE_OPTIONS="--max-old-space-size=4096" TURBO_CONCURRENCY=1 bunx turbo run build:web --filter=@blinko/frontend; \
    fi
