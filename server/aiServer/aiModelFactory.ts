...
export class AiModelFactory {
  ...
  static async queryVector(query: string, accountId: number, _topK?: number) {
    ...
    try {
      ...
      console.error('[AI] Failed to generate query embedding, skipping vector context:', msg);
      return { notes: [], aiContext: '' };
    }
  ...
  static async GetProvider() {
    ...
    let normalizedModelKey = String(mainModel.modelKey || '').replace(/^models\//, '');
    // Some Gemini model IDs are exposed under the "-latest" alias. Accept common shorthand.
    if (mainModel.provider.provider?.toLowerCase?.() === 'google' || mainModel.provider.provider?.toLowerCase?.() === 'gemini') {
      if (normalizedModelKey === 'gemini-1.5-flash') normalizedModelKey = 'gemini-1.5-flash-latest';
      if (normalizedModelKey === 'gemini-1.5-pro') normalizedModelKey = 'gemini-1.5-pro-latest';
    }
    ...
    if (embeddingModel) {
      ...
      try {
        console.log('[AI] Embedding config:', {
          provider: embeddingConfig.provider,
          modelKey: embeddingConfig.modelKey,
          baseURL: embeddingConfig.baseURL
        });
        embeddings = await embeddingProvider.getEmbeddingModel(embeddingConfig);
      } catch (e: any) {
        const msg = e?.message || String(e);
        console.error('[AI] Failed to initialize embeddings:', {
          provider: embeddingConfig.provider,
          modelKey: embeddingConfig.modelKey,
          baseURL: embeddingConfig.baseURL,
          error: msg
  ...
  static async BaseChatAgent({ withTools = true, withOnlineSearch = false }: { withTools?: boolean; withOnlineSearch?: boolean }) {
    ...
    // Gemini v1 REST streaming rejects the `systemInstruction` field.
    // Mastra maps `instructions` to that field for Google/Gemini models, so omit it.
    const llmProviderName = (provider as any)?.llmProviderName;
    const shouldOmitInstructions = llmProviderName === 'google' || llmProviderName === 'gemini';

    const BlinkoAgent = new Agent({
      name: 'Blinko Chat Agent',
      ...(shouldOmitInstructions
        ? {}
        : {
            instructions:
              `Today is ${dayjs().format('YYYY-MM-DD HH:mm:ss')}\n` +
