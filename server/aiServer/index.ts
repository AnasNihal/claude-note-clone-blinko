...
export class AiService {
  ...
  static async embeddingUpsert({ id, content, type, createTime, updatedAt }: { id: number; content: string; type: 'update' | 'insert'; createTime: Date; updatedAt?: Date }) {
    try {
      ...
      if (!Embeddings) {
        return { ok: false, msg: 'No embeddings model config' };
      }
  ...
  static async completions({
    ...
    try {
      ...
      // NOTE:
      // For Gemini v1 streaming endpoints, the underlying AI SDK maps `role: 'system'`
      // messages to a top-level `systemInstruction` field. The v1 REST endpoint rejects
      // that field with: "Unknown name \"systemInstruction\"".
      // To keep Gemini v1 working, we fold system context into the first user message.
      const preambleParts: string[] = [];

      // Preserve the global prompt / default instructions while avoiding `systemInstruction`.
      ...
      preambleParts.push(
        (globalConfig as any)?.globalPrompt
          ? String((globalConfig as any).globalPrompt)
          : defaultInstructions,
      );

      preambleParts.push(`Current user name: ${ctx.name}`);
      if (systemPrompt) {
        preambleParts.push(systemPrompt);
      }

      // Remove any pre-existing system messages to avoid triggering `systemInstruction`.
      conversations = (conversations || []).filter((m: any) => m?.role !== 'system');

      let ragNote: any[] = [];
      if (withRAG) {
        let { notes, aiContext } = await AiModelFactory.queryVector(question, Number(ctx.id));
        ragNote = notes;
        if (ragNote.length || aiContext) {
          preambleParts.push(
            `This is the note content ${ragNote.map((i) => i.content).join('\n')} ${aiContext}`,
          );
        }
      }
      ...
      conversations.push({
        role: 'user',
        content: `${preamble}${question}`,
      });

      console.log(conversations, 'conversations');

      // Gemini v1 streaming is strict about request JSON fields and also conflicts with Mastra's
      // instruction handling. For google/gemini providers, stream directly via ai-sdk to avoid
      // emitting unsupported `systemInstruction` and avoid Mastra runtime errors.
      const provider = await AiModelFactory.GetProvider();
      const llmProviderName = (provider as any)?.llmProviderName;
      const isGemini = llmProviderName === 'google' || llmProviderName === 'gemini';
      if (isGemini) {
      ...
      if (anyErr?.name === 'AI_APICallError') {
        console.error('[AI] AI_APICallError details:', {
        ...
        console.error('[AI] completions error:', anyErr);
      }
