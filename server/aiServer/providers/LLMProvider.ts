    ...
    switch (config.provider.toLowerCase()) {
      ...
      case 'google':
        const normalizedGoogleBaseURL = (() => {
          ...
          if (!raw) return 'https://generativelanguage.googleapis.com/v1';
          const withoutTrailingSlash = raw.replace(/\/+$/, '');
          // If the user entered just the host, default to /v1.
          if (withoutTrailingSlash === 'https://generativelanguage.googleapis.com') {
        ...
        return createGoogleGenerativeAI({
          apiKey: config.apiKey,
          baseURL: normalizedGoogleBaseURL,
          fetch: this.proxiedFetch
        }).languageModel(config.modelKey);
