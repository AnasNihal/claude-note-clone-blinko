...
export const getGlobalConfig = async ({ ctx, useAdmin = false }: { ctx?: Context, useAdmin?: boolean }) => {
  ...
  const readableGlobalKeysForAuthedUsers = new Set<string>([
    // AI model selection (non-secret IDs)
    'mainModelId',
    'embeddingModelId',
    'voiceModelId',
    'rerankModelId',
    ...
    // AI behavior / prompts (non-secret)
    'isUseAiPostProcessing',
    'aiCommentPrompt',
    'aiTagsPrompt',
    'aiPostProcessingMode',
    'aiCustomPrompt',
    'aiSmartEditPrompt',
    'globalPrompt',

    // RAG tuning (non-secret)
    'embeddingTopK',
    'embeddingScore',
  ...
  const globalConfig = configs.reduce((acc, item) => {
    ...
    // Global (non-user) configs are admin-only by default, but some are safe/needed for
    // normal authenticated users (e.g., AI model selection + prompts).
    if (!isSuperAdmin && !item.userId) {
      if (!userId) return acc;
      if (!readableGlobalKeysForAuthedUsers.has(item.key)) return acc;
    }
